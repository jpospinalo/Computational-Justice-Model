...
; Linear Public Game: Distribution of resources in cooperation process
...
to setup-players
 ...
 ; history records of allocation, demand, provision, appropiation and compliant rounds
  ...
  ask players [
    ...
    set satisfaction-level precision (0.5) 6  ;we can use a random satisfaction level
    set m 0
    set m-threshold threshold
  ]


  ask players[set t-who who]


  ; Determines the resources it has avaiable
   ask players [set current-resources precision random-float 1 8 ]

  ; Determines its need of resources
   ...
   (foreach agent-set
    [ [i] ->
       ask player i [
        set current-need  precision (current-resources + random-float (1 - current-resources) * scarcity) 7
    ]
  ])


  ; Make a demand of resources
  ask players [set current-demand current-need]

  ;Make a provision of resources
  ask players [set current-provision current-resources]

  set resource-1 (sum([current-provision] of players))
  set resource-2 (sum([current-resources] of players))
...
to go
  ...
  set total-resource-2 total-resource-2 + resource-2


  if (ticks >= 2500 or count(players) < 3) [stop]



;##################################################################################
  ; this process was necessary to adapt the PD model to LPG

  let agent-set ([who] of players)

  (foreach agent-set
  [ [i] ->
    ask player i [  if (m > m-threshold) [ die ]
    ]
  ])

  set agent-set ([who] of players)

  (foreach agent-set (range (count(players) ))
    [ [i tem] -> ask player i [ set t-who tem ]])
...
to play-a-round
  ...
  ;update-resource-1
  ;update-resource-2
  ifelse (ticks > 1 and count(players) > 1) [set gini-index compute-gini-index][set gini-index 0]
end
...
to update-player-parameterers
  ...
  ;ask players [ if (m > m-threshold) [die]]
   ...
   ask players [
    set total-resources (total-resources + current-resources)
 ...
 ; let agents-need2 (n-values count(players)
 ...
 (foreach agent-set
  ...
  ;ask players [set current-provision current-resources]
   ...
   ask players [
    let p (random-float 1)
    ifelse (cheat-on = true and cheating-method = "provision" and p < p-cheat)[
      set current-provision current-resources * (random-float 1)
      ][
      set current-provision current-resources
      ]
    ]
...
to get-payoff-game1 ;; game in which everyone cooperate (it has a greater payoff)
  ...
  let R (current-appropriation + (current-resources - current-provision))
  ifelse (R >= current-need) [
    set utility (utility + (a * current-need + b * (R - current-need)))
    set label precision utility 1
    ][
    set utility (utility + (a * R - c * (current-need - R)))
    set label precision utility 1
...
to update-satisfaction-level
  ...
  ; his needs
  ifelse ((current-allocation >= current-demand)) [
...
to-report borda-count-SO
  ...
  ;show "length"
  ;show (length t-equality-satisfaction)
...
;#############################################################################
 ...
 ;#############################################################################
  ...
  set w1 (w1b + w1b * ((borda-w1b - avg-borda-w) / total-borda-w))
...
;#############################################################################
  ...
  (foreach t-equality-satisfaction (sort-on [(- borda-points)] players)
    [[x y] -> if (([t-who] of x) != ([t-who] of y)) [set hamming-w1 (hamming-w1 + 1)]])

  (foreach t-equality-allocation (sort-on [(- borda-points)] players)
    [[x y] -> if (([t-who] of x) != ([t-who] of y)) [set hamming-w1b (hamming-w1b + 1)]])

  (foreach t-need (sort-on [(- borda-points)] players)
    [[x y] -> if (([t-who] of x) != ([t-who] of y)) [set hamming-w2 (hamming-w2 + 1)]])

  (foreach t-productivity-avg-provision (sort-on [(- borda-points)] players)
    [[x y] -> if (([t-who] of x) != ([t-who] of y)) [set hamming-w3 (hamming-w3 + 1)]])

  (foreach t-effort (sort-on [(- borda-points)] players)
    [[x y] -> if (([t-who] of x) != ([t-who] of y)) [set hamming-w4 (hamming-w4 + 1)]])

  (foreach t-social-utility (sort-on [(- borda-points)] players)
    [[x y] -> if (([t-who] of x) != ([t-who] of y)) [set hamming-w5 (hamming-w5 + 1)]])

  (foreach t-supply-and-demand (sort-on [(- borda-points)] players)
    [[x y] -> if (([t-who] of x) != ([t-who] of y)) [set hamming-w6 (hamming-w6 + 1)]])
...
;##################################################################################
  ...
  let population-size 20
   show "starting"
  let initial-population matrix:make-constant population-size 7 0
  let new-population matrix:make-constant population-size 7 0
  let fitness-population matrix:make-constant population-size 1 0
  let i 0
  while [i < population-size][
    let j 0
    let chromosome  (n-values 7 [ k -> random-float 1])
    let total sum(chromosome)
    while [j < 7][
      matrix:set initial-population i j ((item j chromosome) / total )
      set j (j + 1)
    ]
   set i (i + 1)
  ]
...
;##################################################################################
  ...
  while[k < population-size][
   ; show "_____________  TEST ____________"
   ; show k
    setup
    let individual (matrix:get-row initial-population k)
    set w1  (item 0 individual)
    set w1b (item 1 individual)
    set w2  (item 2 individual)
    set w3  (item 3 individual)
    set w4  (item 4 individual)
    set w5  (item 5 individual)
    set w6  (item 6 individual)
   ...
   matrix:set fitness-population k 0  (count(players with [cheat-on = false]) )
   set k (k + 1)
  ]

  ;show "------------fitness population---------------------"
  ;show fitness-population
  show "------------maximun - fitnesss---------------------"
  show max(matrix:get-column fitness-population 0)
  show "------------best - individual ---------------------"
  show matrix:get-row initial-population (position max(matrix:get-column fitness-population 0) (matrix:get-column fitness-population 0))
...
;##################################################################################
  ...
  while[flag < (population-size / 2)][

    let parents (matrix:get-column fitness-population 0)

    let aoo (max parents)
    let index-max position aoo parents
    matrix:set-row new-population flag (matrix:get-row initial-population index-max)
    matrix:set fitness-population index-max 0 (-1)
    set flag (flag + 1)
  ]
...
;##################################################################################
  ...
  let rep (population-size / 2)
  let val2 0

  while[rep < population-size ][


    let r random 7
    let r-float random-float 1

    let p1 item r (matrix:get-row new-population val2)
    let p2 item r (matrix:get-row new-population (val2 + 1))

    set p1 (p1 - r-float * (p1 - p2) )
    set p2 (p2 + r-float * (p1 - p2) )
    ...
    let offspring-1 n-values 7 [0]
    let offspring-2 n-values 7 [0]


    set offspring-1 (replace-item r offspring-1 p1)
    set offspring-2 (replace-item r offspring-2 p2)


    let flag1 0
    while[flag1 < r][
      set offspring-1 (replace-item flag1 offspring-1 (matrix:get new-population val2 flag1 ))
      set offspring-2 (replace-item flag1 offspring-2 (matrix:get new-population (val2 + 1) flag1))
      set flag1 (flag1 + 1)
    ]

    set flag1 6
    while[ flag1 > r][
      set offspring-1 (replace-item flag1 offspring-1 (matrix:get new-population (val2 + 1) flag1 ))
      set offspring-2 (replace-item flag1 offspring-2 (matrix:get new-population val2 flag1))
      set flag1 (flag1 - 1)
   ]

    set flag1 0
    let total-of1 sum(offspring-1)
    let total-of2 sum(offspring-2)
    while[flag1 < 7][
      set offspring-1 (replace-item flag1 offspring-1 ((item flag1 (offspring-1)) / total-of1))
      set offspring-2 (replace-item flag1 offspring-2 ((item flag1 (offspring-2)) / total-of2))
      set flag1 (flag1 + 1)
   ]

   matrix:set-row new-population rep offspring-1
   matrix:set-row new-population (rep + 1) offspring-2
  ...
  set initial-population new-population

  set all-rounds (all-rounds + 1)
]
...
to initialize-population
  ...
  let individual (matrix:get-row initial-population 0)
...
702
44
Linear Public Game  - Tragedy of the commons \n    
15
0.0
1
...
allocation-method
allocation-method
"legitimate-claims-S0" "legitimate-claims" "legitimate-claims-evolutionary" "ostrom-random-allocation" "greedy-random-allocation" "equal-allocation" "max-min-allocation"
1
...
cheating-method
cheating-method
"demand" "appropriation" "provision"
0
...
172
NC-Agents
count(players) with [cheat-on = true]
17
1
11
...
105
C-Agents
count(players) with [cheat-on = false]
17
1
11
...
706
U-c
mean( [utility ] of players with [cheat-on = false])
4
1
11
...
706
U-s
mean( [utility ] of players with [cheat-on = true])
4
1
11
...
792
Total U
sum( [utility ] of players with [cheat-on = false])
4
1
11
...
@#$#@#$#@
<experiments>
  <experiment name="Scenario 1 -LPG -legitimateClaims" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    ...
    <metric>count(players) with [cheat-on = false]</metric>
    <metric>count(players) with [cheat-on = true]</metric>
  ...
  <experiment name="Scenario 1 -LPG -RandomAllocation" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>precision (gini-index) 4</metric>
    <metric>precision (mean( [utility / ticks] of players with [cheat-on = false])) 4</metric>
    ...
    <enumeratedValueSet variable="allocation-method">
      <value value="&quot;ostrom-random-allocation&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="threshold">
      <value value="3"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="Scenario 1 -LPG -EqualAllocation" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>precision (gini-index) 4</metric>
    <metric>precision (mean( [utility / ticks] of players with [cheat-on = false])) 4</metric>
    ...
    <enumeratedValueSet variable="allocation-method">
      <value value="&quot;equal-allocation&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="threshold">
      <value value="3"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="Scenario 1 -LPG - Radonm-Apropriation" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>precision (gini-index) 4</metric>
    <metric>precision (mean( [utility / ticks] of players with [cheat-on = false])) 4</metric>
    ...
    <enumeratedValueSet variable="allocation-method">
      <value value="&quot;greedy-random-allocation&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="threshold">
      <value value="3"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="Scenario 2 -LPG - LegitimateClaims-Demand" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>precision (gini-index) 4</metric>
    <metric>precision (mean( [utility / ticks] of players with [cheat-on = false])) 4</metric>
  ...
  <experiment name="Scenario 2 -LPG - LegitimateClaims-Apropriation" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>precision (gini-index) 4</metric>
    <metric>precision (mean( [utility / ticks] of players with [cheat-on = false])) 4</metric>
  ...
  <experiment name="Scenario 2 -LPG - LegitimateClaims-Provision" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>precision (gini-index) 4</metric>
    <metric>precision (mean( [utility / ticks] of players with [cheat-on = false])) 4</metric>
  ...
  <experiment name="Scenario 4 -LPG - GA (Beta 0.1)" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>precision (gini-index) 4</metric>
    <metric>precision (mean( [utility / ticks] of players with [cheat-on = false])) 4</metric>
  ...
  <experiment name="Scenario 4 -LPG - GA (Beta 0.2)" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>precision (gini-index) 4</metric>
    <metric>precision (mean( [utility / ticks] of players with [cheat-on = false])) 4</metric>
  ...
  <experiment name="Scenario 4 -LPG - GA (Beta 0.3)" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>precision (gini-index) 4</metric>
    <metric>precision (mean( [utility / ticks] of players with [cheat-on = false])) 4</metric>
  ...
  <experiment name="Scenario 4 -LPG - GA (Beta 0.4)" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>precision (gini-index) 4</metric>
    <metric>precision (mean( [utility / ticks] of players with [cheat-on = false])) 4</metric>
  ...
  <experiment name="Scenario 4 -LPG - GA (Beta 0.6)" repetitions="50" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>precision (gini-index) 4</metric>
    <metric>precision (mean( [utility / ticks] of players with [cheat-on = false])) 4</metric>
